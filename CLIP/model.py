...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, cnn, normalize: bool = True):
        cast_dtype = self.transformer.get_cast_dtype()
        x = self.token_embedding(text).to(cast_dtype)  # [batch_size, n_ctx, d_model] 245,77,768
        if cnn != '':
            # cnn = cnn.unsqueeze(0).unsqueeze(0).repeat(x.shape[0], 1, 1)
            # x = torch.cat((cnn, x), dim=1)
            x = x.clone()
            for i in range(x.shape[0]):
                x[i][0] = cnn

        # cos = cosine_similarity(torch.mean(x, dim=1), cnn)
